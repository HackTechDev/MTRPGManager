...
class Scenario extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('title, scenariotag, synopsis, scenario_id', 'required'),
			array('scenario_id', 'numerical', 'integerOnly'=>true),
			array('title, scenariotag', 'length', 'max'=>128),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, title, scenariotag, synopsis, scenario_id', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'sceneTrees' => array(self::HAS_MANY, 'SceneTree', 'scenario_id'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('scenariotag',$this->scenariotag,true);
